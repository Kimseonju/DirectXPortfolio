...
bool CResourceManager::Init()
{
	...
	//DirectWrite 팩토리 생성
	//DWRITE_FACTORY_TYPE_SHARED 공유
	if (FAILED(DWriteCreateFactory(DWRITE_FACTORY_TYPE_SHARED, _uuidof(m_WriteFactory), (IUnknown**)&m_WriteFactory)))
		return false;
	...
	CreateFontFile("DungreedFont", TEXT("AaCassiopeia.ttf"));
...
void CResourceManager::SetParticleUpdateShader(const std::string& Name, const std::string& ShaderName)
{
	CParticleSystem* Particle = FindParticle(Name);
	if (!Particle)
		return;
	Particle->SetUpdateShader(ShaderName);
}

CParticleUpdateShader* CResourceManager::GetParticleUpdateShader(const std::string& Name)
{
	CParticleSystem* Particle = FindParticle(Name);
	if (!Particle)
		return nullptr;
	return Particle->GetUpdateShader();
}
...
bool CResourceManager::CreateFontFile(const std::string& Name, const TCHAR* FileName, const std::string& PathName)
{
	...
	//지정된 글꼴 파일의 모든 글꼴에 대한 참조를 추가합니다.
	//글꼴 파일을 구문 분석하여 글꼴과 해당 속성을 결정합니다.
	...
	//글꼴을 만들고 가장 가깝게 일치하는 글꼴을 선택하고 필터링하는 데 사용되는 글꼴 집합입니다.
...
bool CResourceManager::CreateTextFormat(const std::string& Name, const TCHAR* FontName, int Weight, float Size, const TCHAR* LocalName, int Stretch)
{
	...
	//텍스트 레이아웃에 사용되는 텍스트 형식 객체를 생성합니다.
	if (FAILED(m_WriteFactory->CreateTextFormat(FontName, nullptr, (DWRITE_FONT_WEIGHT)Weight,
		DWRITE_FONT_STYLE_NORMAL, (DWRITE_FONT_STRETCH)Stretch, Size, LocalName, &Font)))
		return false;
