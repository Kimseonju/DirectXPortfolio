...
{
	{
		...
		{
			iter->second->Release();
			//ReleaseMesh을 체크해서 만약 카운트가1이면 매니저만 가지고있으므로 지우게끔하기
...
void CSceneResource::SetParticleUpdateShader(const std::string& Name, const std::string& ShaderName)
{
	CParticleSystem* Particle = FindParticle(Name);
	if (!Particle)
		return;
...
CParticleUpdateShader* CSceneResource::GetParticleUpdateShader(const std::string& Name)
{
	CParticleSystem* Particle = FindParticle(Name);
	if (!Particle)
		return nullptr;
