...
void CColliderBox2D::SetExtent(float Width, float Height)
{
...
void CColliderBox2D::SetLine2D(Vector2 src, Vector2 dest)
{
	float length = (src - dest).Length();

	m_Info.Length[0] = length;
	m_Info.Length[1] = 0.1f;

	SetWorldScale(m_Info.Length[0] * 2.f, m_Info.Length[1] * 2.f, 1.f);
	SetWorldRotationZ(src.GetAngle(dest));
}
