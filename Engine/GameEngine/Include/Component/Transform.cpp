...
void CTransform::InheritRot()
{
    ...
    // 위에서 구해준 최종 회전행렬을 이용해서 x, y, z 축을 곱하여 회전된 월드상에서의 최종 축 구하기
	//축을 회전시키는 이유는 30도만큼 회전하면 그만큼 축이 움직이게되는데
	//월드상에서의 고유한 축을 갖고있으면 이동할때 처리하기 편할것이기 때문이다.
	//바라보는방향이라던지 등등 이미 구해진 상태이기 때문이다.
    for (int i = 0; i < AXIS_END; ++i)
    {
...
void CTransform::SetRelativeRotation(const Vector3& Rot)
{
    ...
    //최종 회전행렬을 이용해서 x,y,z축을 곱하여 회전된 월드상에서의
    //최종축 구하기
...
void CTransform::SetWorldPos(const Vector3& Pos)
{
    ...
    if (m_pParent)
    {
        ...
        // 역행렬을 구한다.
        // 구하는이유는 상대값을 가져오기 위해서
        // World행렬을 구할때 matRot를 곱했는데
        //상대값가져오려면 World행렬 구하기 이전으로 돌려야되므로
        // 역행렬을 곱함
        matRot.Inverse();
...
void CTransform::SetTransform()
{
    ...
    m_pCBuffer->SetHorizontalReverse2DEnable(m_HorizontalReverse2DEnable);
    m_pCBuffer->SetVerticalReverse2DEnable(m_VerticalReverse2DEnable);
